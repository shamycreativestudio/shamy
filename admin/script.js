...
async function loadBriefs() {
  ...
  try {
    ...
    briefsList.innerHTML =
      '<div class="error">❌ Error al cargar briefs. Verifica tu conexión.</div>';
    throw error;
  }
}
...
function renderBriefs() {
  briefCount.textContent = `${briefs.length} brief${
    briefs.length !== 1 ? "s" : ""
  }`;
        ...
        <div class="brief-card" data-id="${brief.id}">
            <div class="brief-header">
                <div class="brief-title">
                    ...
                    <p>${brief.empresa || "Sin empresa"} • ${
        brief.email || "Sin email"
      }</p>
                </div>
                <div class="brief-meta">
                    <div>${formatDate(brief.fecha)}</div>
                    <span class="brief-status ${(
                      brief.estado || "Nuevo"
                    ).toLowerCase()}">${brief.estado || "Nuevo"}</span>
                </div>
            </div>
            <div class="brief-actions">
                <button class="btn-view" onclick="viewBrief('${
                  brief.id
                }')">👁️ Ver detalles</button>
                <button class="btn-delete" onclick="deleteBrief('${
                  brief.id
                }')">🗑️ Eliminar</button>
            </div>
        </div>
...
function viewBrief(id) {
  ...
  detailContent.innerHTML = `
        ...
        <div class="detail-field">
            <label>Estado:</label>
            <p><span class="brief-status ${(
              brief.estado || "Nuevo"
            ).toLowerCase()}">${brief.estado || "Nuevo"}</span></p>
        </div>
    `;
